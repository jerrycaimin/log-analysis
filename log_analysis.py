    ...
    for target_folder in target_folders: 
        ...
        for refine_log in refine_logs:
            ...
            for item in items:
                ...
                _write_refine_log(os.path.join(target_folder, filepath), rule, datetime_exp, datetime_convertor, node_name, writer)
...
def _parse_issue(target_path, issues, warning_hittimes=None, define_times=None, output_file=None):
    ...
    for issue in issues:
        ...
        _write_log(output_file, "==================Start to check issue " + str(issue_num) + ": " +issue_name+"==============")
        ...
        if define_times is not None:
            if counter >= define_times:
                ...
                _write_log(output_file, "The issue [" + issue_name+"] is highly happened.\nPlease refer the advice:\n" + advice)
        elif warning_hittimes is not None:
            if counter >= warning_hittimes:
                print("The issue [" + issue_name+"] is probably happened, please check the log to determine. If existed, follow the advice:%s\n" % advice)

        _write_log(output_file, "==================Finish to check issue " + str(issue_num) + ": " +issue_name+"==============")
        _write_log(output_file, "============================================================================================")
        _write_log(output_file, "\n")
